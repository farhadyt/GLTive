...
import { PackagePlus, ChevronRight, Loader2 } from "lucide-react";
import { useLookupItems, useReceiveQuantity } from "../hooks/useStockQueries";
import toast from "react-hot-toast";

export function ReceivePage() {
  const { t } = useTranslation();
  const { data: items, isLoading: itemsLoading } = useLookupItems();
  const mutation = useReceiveQuantity();
    ...
    <>
      <div className="mb-8">
        <nav className="flex items-center gap-2 text-[var(--color-outline)] text-xs font-medium uppercase tracking-wider mb-2">
          ...
          <span className="text-[var(--color-primary)]">{t("nav.receive")}</span>
        </nav>
        <h1 className="text-3xl font-bold text-white tracking-tight">{t("nav.receive")}</h1>
      </div>
      ...
      <div className="max-w-lg">
        <form onSubmit={handleSubmit} className="bg-[var(--surface-container-low)] rounded-2xl p-8 space-y-5 border border-white/5">
            ...
            >
              ...
              {!itemsLoading && items?.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.display_name || item.internal_code || item.id.slice(0, 8)}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-[var(--color-on-surface-variant)] uppercase tracking-wider">
              {t("common.description")} ({t("common.qty")})
            </label>
            <input
              type="number"
              step="0.01"
