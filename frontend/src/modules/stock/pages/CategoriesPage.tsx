...
import { Tags, ChevronRight, Plus, Pencil } from "lucide-react";
import { useCategories, useCreateCategory, useUpdateCategory } from "../hooks/useStockQueries";
      ...
      <div className="bg-[var(--surface-container-low)] rounded-2xl overflow-hidden">
        {isLoading ? (
              ...
              <tbody className="text-sm">
                {data.items.map((cat, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-[var(--color-outline)] truncate max-w-[200px]">
                      {cat.description || "—"}
                    </td>
          ...
          <div className="p-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-[var(--color-outline)]">{t("common.coming_soon_desc")}</p>
          </div>
        )}
      </div>
...
function CategoryModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        await updateMutation.mutateAsync({ id: category!.id, data: { code, name, description } });
        toast.success(t("common.save") + " ✓");
      } else {
        await createMutation.mutateAsync({ code, name, description });
      ...
      <div className="relative z-10 w-full max-w-md bg-[var(--surface-container)] rounded-2xl border border-white/5 shadow-2xl">
        <div className="p-6 border-b border-white/5">
          <h2 className="text-lg font-bold text-white">
            {isEdit ? t("common.edit") : t("common.create")} — {t("nav.categories")}
          </h2>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              required
              disabled={isEdit}
              className="w-full px-4 py-3 text-sm rounded-xl bg-[var(--surface-container-lowest)] text-[var(--color-on-surface)] border border-white/5 focus:outline-none focus:border-[var(--color-primary-container)] disabled:opacity-50"
            />
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 text-sm font-medium text-[var(--color-on-surface-variant)] bg-[var(--surface-container-highest)] rounded-xl hover:bg-[var(--surface-bright)] transition-colors"
            >
              {t("common.cancel")}
            </button>
            <button
              type="submit"
              disabled={isPending}
              className="px-5 py-2.5 text-sm font-bold btn-primary-gradient rounded-xl disabled:opacity-50"
            >
