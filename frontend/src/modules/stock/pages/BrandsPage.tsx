...
import { Bookmark, ChevronRight, Plus, Pencil } from "lucide-react";
import { useBrands, useCreateBrand, useUpdateBrand } from "../hooks/useStockQueries";
...
export function BrandsPage() {
  ...
  const { data, isLoading } = useBrands(page);
      ...
      <div className="mb-8 flex justify-between items-end">
        <div>
          <nav className="flex items-center gap-2 text-[var(--color-outline)] text-xs font-medium uppercase tracking-wider mb-2">
            ...
            <span className="text-[var(--color-primary)]">{t("nav.brands")}</span>
          </nav>
          <h1 className="text-3xl font-bold text-white tracking-tight">{t("nav.brands")}</h1>
        </div>
      ...
      <div className="bg-[var(--surface-container-low)] rounded-2xl overflow-hidden">
        {isLoading ? (
              ...
              <tbody className="text-sm">
                {data.items.map((item, i) => (
                  <tr key={item.id} className={`hover:bg-white/5 transition-colors ${i % 2 ? "bg-white/[0.02]" : ""}`}>
                    ...
                    <td className="px-6 py-4 text-[var(--color-primary)] text-xs">{item.website || "—"}</td>
          ...
          <div className="p-16 flex flex-col items-center justify-center text-center">
            <Bookmark className="w-16 h-16 text-[var(--color-outline)]/30 mb-6" />
            <h3 className="text-xl font-bold text-white mb-2">{t("common.no_data")}</h3>
          </div>
        )}
      </div>

      {modalOpen && <BrandModal brand={editItem} onClose={() => setModalOpen(false)} />}
    </>
  );
}
...
function BrandModal({ brand, onClose }: { brand: Brand | null; onClose: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        await updateMut.mutateAsync({ id: brand!.id, data: { name, description, website } });
      } else {
        await createMut.mutateAsync({ name, description, website });
      }
      toast.success("✓");
      onClose();
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md bg-[var(--surface-container)] rounded-2xl border border-white/5 shadow-2xl">
        <div className="p-6 border-b border-white/5">
          <h2 className="text-lg font-bold text-white">{isEdit ? t("common.edit") : t("common.create")} — {t("nav.brands")}</h2>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-[var(--color-on-surface-variant)] uppercase tracking-wider">Website</label>
            <input value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://" className="w-full px-4 py-3 text-sm rounded-xl bg-[var(--surface-container-lowest)] text-[var(--color-on-surface)] border border-white/5 focus:outline-none focus:border-[var(--color-primary-container)]" />
          </div>
